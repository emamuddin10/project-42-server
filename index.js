...
async function run() {
  try {
    ...
    // filtering biodata
    app.get("/biodataFiltering", async (req, res) => {
    ...
    app.post("/premium-requests", async (req, res) => {
      ...
      try {
        ...
        if (existingRequest) {
          ...
          return res
            .status(400)
            .json({ message: "A request with the same email already exists." });
        }
    ...
    app.patch("/premium-requests/approve/:id", async (req, res) => {
      const id = req.params.id;
      const request = await premiumRequestCollection.findOne({
        _id: new ObjectId(id),
      });
      ...
      try {
        ...
        if (existingRequest) {
          ...
          if (existingRequest.status === "Pending") {
            ...
            return res
              .status(200)
              .json({ message: "Existing request updated successfully." });
          } else {
            console.log("Existing request already approved. No action taken.");
            return res
              .status(400)
              .json({
                message: "Existing request already approved. No action taken.",
              });
          }
        }
    ...
    app.get("/premium-requests/:id", async (req, res) => {
      ...
      try {
        const result = await premiumRequestCollection.findOne({
          _id: new ObjectId(id),
        });
        if (!result) {
